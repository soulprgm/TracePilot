...
8. **数据治理**：默认保留 30 天，每天自动清理过期数据；平台健康检查不会污染业务指标。
9. **云端部署**：Render Blueprint 定义后端、数据库、Jaeger、Collector 和三个微服务。
10. **质量保障**：GitHub Actions 自动测试四个 Java 模块，并提供云端验收脚本。
11. **AI 根因分析**：可分析最新链路或任意选中链路，输出严重程度、摘要、可能根因、证据和处理建议；配置 OpenAI 密钥后由模型生成，未配置时明确显示基础诊断模式。
...
- 点击 **Analyze latest trace**，或在链路详情中点击 **Analyze with AI**，可查看根因分析结果。
